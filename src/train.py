...
def train_on_batch(ARGS, input_tensor, target_tensor, target_number_tensor, i3d_vec, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, device):
    encoder.train()
    decoder.train()
...
def train(ARGS, encoder, decoder, transformer, train_generator, val_generator, exp_name, device, encoder_optimizer=None, decoder_optimizer=None):
    ...
    loss_plot_file_path = '/data1/louis/experiments/{}/{}_lossplot.png'.format(exp_name, exp_name)
    ...
    for epoch_num in range(ARGS.max_epochs):
        ...
        for iter_, training_triplet in enumerate(val_generator):
            ...
            video_ids = training_triplet[4].float().to(device)
            i3d_vec = training_triplet[5].float().to(device)
            new_val_loss, new_val_norm_loss = eval_on_batch(ARGS, input_tensor, target_tensor, target_number, i3d_vec, video_ids=video_ids, encoder=encoder, decoder=decoder, transformer=transformer, dec_criterion=criterion, mlp_dict=mlp_dict, json_data_dict=json_data_dict, device=device)
...
def eval_on_batch(ARGS, input_tensor, target_tensor, target_number_tensor, i3d_vec, video_ids=None, encoder=None, decoder=None, transformer=None, regressor=None, dec_criterion=None, reg_criterion=None, mlp_dict=None, json_data_dict=None, device='cpu'):

...
def load_vid_from_id(vid_id):
    return np.load(f'/data1/louis/data/frames/vid{vid_id}.npz')['arr_0']
...
def load_data_lookup(json_data, vgg_lookup_table, i3d_lookup_table, batch_size, shuffle):
    new_data = LookupDataset(json_data, vgg_lookup_table, i3d_lookup_table)
    new_data_loaded = data.DataLoader(new_data, batch_size=batch_size, shuffle=shuffle, drop_last=True)

    return new_data_loaded

def get_split_dls(json_data,splits,batch_size,shuffle,i3d):
    train_data = json_data[0:splits[0]]
    val_data = json_data[splits[0]:splits[1]]
    test_data = json_data[splits[1]:splits[2]+1]
    train_table = vgg_lookup_table_from_ids([dp['video_id'] for dp in train_data])
    train_i3d_table = i3d_lookup_table_from_ids([dp['video_id'] for dp in train_data]) if i3d else None  
    train_dl = load_data_lookup(train_data,train_table,train_i3d_table,batch_size,shuffle)
    val_table = vgg_lookup_table_from_ids([dp['video_id'] for dp in val_data])
    val_i3d_table = i3d_lookup_table_from_ids([dp['video_id'] for dp in val_data]) if i3d else None
    val_dl = load_data_lookup(val_data, val_table,val_i3d_table,batch_size,shuffle)
    test_table = vgg_lookup_table_from_ids([dp['video_id'] for dp in test_data])
    test_i3d_table = i3d_lookup_table_from_ids([dp['video_id'] for dp in test_data]) if i3d else None
    test_dl = load_data_lookup(test_data,test_table,test_i3d_table,batch_size,shuffle)
